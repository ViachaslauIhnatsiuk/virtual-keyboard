  ...
  document.querySelectorAll('.key').forEach((item) => item.addEventListener('click', (e) => {
    ...
    if (OUTPUT.selectionEnd - OUTPUT.selectionStart > 0) {
      ...
      OUTPUT.textContent = OUTPUT.textContent.slice(0, position)
        + e.target.textContent + OUTPUT.textContent.slice(position);
    }
...
const renderCapslockKeys = () => {
  document.querySelectorAll('.key').forEach((key) => {
    ...
    if (localStorage.language === 'ru') {
      ru.forEach((item) => {
        if (key.textContent === item.lowerValue) {
          KEY.textContent = item.upperCapsValue;
        } else if (key.textContent === item.upperValue) {
          KEY.textContent = item.lowerValue;
        }
      });
    } else {
      en.forEach((item) => {
...
const renderShiftLowerLetterKeys = () => {
  document.querySelectorAll('.letter').forEach((key) => {
    ...
    if (localStorage.language === 'ru') {
      ru.forEach((item) => {
        if (key.textContent === item.upperValue) {
          KEY.textContent = item.lowerValue;
        }
      });
    } else {
...
const renderShiftLowerNonLetterKeys = () => {
  document.querySelectorAll('.no-letter').forEach((key) => {
    const KEY = key;
    if (localStorage.language === 'ru') {
      ru.forEach((item) => {
        if (key.textContent === item.upperValue) {
...
const renderShiftUpperLetterKeys = () => {
  document.querySelectorAll('.letter').forEach((key) => {
    ...
    if (localStorage.language === 'ru') {
      ru.forEach((item) => {
        if (key.textContent === item.lowerValue) {
          KEY.textContent = item.upperValue;
        }
      });
    } else {
      en.forEach((item) => {
